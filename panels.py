...
class Panel_Free_Area(bpy.types.Panel):
    ...
    def draw(self, context): 
        ...
        if len(data.rules) > 0:
            item = props.rules[props.active_rule_index]
            if item.show_rule:
                row = layout.row() 
                row.separator()
...
class GI_UL_rules(bpy.types.UIList):
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname):
        if item:
            ...
            col2.label(text=item.description)
            row.operator("gei.show_hide_rule", text='', icon=icon).id = item.id
    ...
    def draw(self, context): 
        ...
        if len(props.result_elements) > 0:
            ...
            if len(props.result_elements) > 0:
                ...
                row = layout.row()
                row.operator("gei.save_results", text="Save Results", icon='FILE_TICK')
                row = layout.row()
